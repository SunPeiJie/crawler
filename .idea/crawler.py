...
class QSBK:
    ...
    def getPageItems(self,pageIndex):
        ...
        if not pageCode:
            print "页面加载失败...."
            return None
        pattern = re.compile(r'<div class="author clearfix">.*?<a href.*?<img.*?title=(.*?)>.*?<div class="content">(.*?)<!--.*?-->.*?<i class="number">(.*?)</i>.*?<i class="number">(.*?)</i>.*?</span>', re.S)
        items = re.findall(pattern,pageCode)     #用来存储每页的段子们
        pageStories = []
        #遍历正则表达式匹配的信息
        for item in items:
            #是否含有图片
